        ...
        for i, pos in enumerate(converted_positions):
            ...
            if self.include_colors and  colors and i <len(colors):
                ...
                if color and len(color) >= 3:  # 确保颜色数据有效
                    def convert_color_component(component):
                        try:
                            #确保是数值类型
                            if isinstance(component, (int, float)):
                                # 将0-1浮点数（可能超过1）映射到0-255范围
                                # 如果值超过1，我们将其视为HDR值，可以限制在255内
                                # 或者乘以255后截断到0-255
                                value = float(component) * 255.0
                                return max(0,min(255,int(round(value))))
                            else:
                                #如果无法转换，返回默认值
                                return 255
                        except Exception as e:  # 忽略任何转换错误
                            print(f"颜色分量装换错误：{component} ->{e}")
                            return 255
                    #转换三个颜色通道
                    channel["c1"] = convert_color_component(color[0])
                    channel["c2"] = convert_color_component(color[1])
                    channel["c3"] = convert_color_component(color[2])

                    if i<3:
                        print(f"点{i}颜色转换：{color}-> R:{channel['c1']},G:{channel['c2']},B:{channel['c3']}")
                else:
                    channel["c1"] = 255
                    channel["c2"] = 255
                    channel["c3"] = 255
