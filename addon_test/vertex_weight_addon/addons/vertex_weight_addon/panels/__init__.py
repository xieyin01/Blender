...
def draw_vertex_weight_overlay():
    ...
    active_group_index = obj.vertex_groups.active_index if obj.vertex_groups.active else -1
...
class VIEW3D_PT_vertex_weight_main(BasePanel, bpy.types.Panel):
    ...
    dummy: bpy.props.FloatProperty(name="Dummy", default=0.0)  # type: ignore
    ...
    def draw(self, context: bpy.types.Context):
        ...
        row.operator(
            ToggleWeightOverlay.bl_idname,
            text="Hide Overlay" if scene.vw_display_weights else "Show Overlay",
        ...
        box = layout.box()
        box.label(
            text=f"Mesh: {obj.name}  |  Verts: {len(mesh.vertices)}",
            icon='MESH_DATA'
        )
        ...
        layout.separator()
        row = layout.row(align=True)
        row.operator(NormalizeWeightsOperator.bl_idname, text="Normalize")
        row.operator(CopyVertexWeightsOperator.bl_idname, text="Copy")
        ...
        for vert in mesh.vertices:
            ...
            valid_groups = [(g.group, g.weight) for g in vert.groups]
            valid_groups.sort(key=lambda x: x[1], reverse=True)

            for group_idx, weight in valid_groups:
                if group_idx < len(obj.vertex_groups):
                    group_name = obj.vertex_groups[group_idx].name
                else:
...
class VIEW3D_PT_vertex_weight_list(BasePanel, bpy.types.Panel):
    ...
    def draw(self, context: bpy.types.Context):
        ...
        selected_count = sum(1 for v in mesh.vertices if v.select)
        if selected_count == 0 or selected_count > 1 or len(obj.vertex_groups) == 0:
